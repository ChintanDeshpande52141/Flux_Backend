...
export async function getSafeToSpend(userId: string) {
  const { start } = getMonthRange();

  const budgetResult = await pool.query(
    'SELECT month_budget, daily_limit FROM user_budgets WHERE user_id = $1',
    [userId]
  );

  const monthBudget = Number(budgetResult.rows[0]?.month_budget ?? 0);
  const dailyLimit = Number(budgetResult.rows[0]?.daily_limit ?? 0);
  ...
  const amount = monthBudget - spent;
...
export async function getSpendingVelocity(userId: string) {
  ...
  const rate = monthBudget > 0 ? Math.round((thisSpent / monthBudget) * 100) : 0;
  const lastRate = monthBudget > 0 ? Math.round((lastSpent / monthBudget) * 100) : 0;
  const trend = rate >= lastRate ? 'up' : 'down';
  const percentChange = lastRate > 0 ? Math.abs(Math.round(((rate - lastRate) / lastRate) * 100)) : 0;

  let tip = 'You\'re on track. Keep it up!';
  if (rate > 80) tip = 'High spending velocity! Consider cutting discretionary expenses.';
  else if (rate > 60) tip = 'Your velocity is moderate. Try reducing dining out.';
  else if (rate < 30) tip = 'Great job! You\'re well within your budget this month.';
...
export async function getSpendingPulse(userId: string) {
...
export async function getSpendingAnalysis(userId: string) {
  ...
  const percentUnder = weekBudget > 0
  ...
  const catTotal = catResult.rows.reduce((s: number, r: { total: string }) => s + Number(r.total), 0);
  const categoryBreakdown = catResult.rows.map((r: { category: string; total: string }) => ({
    label: CATEGORY_LABELS[r.category] ?? r.category,
    percent: catTotal > 0 ? Math.round((Number(r.total) / catTotal) * 100) : 0,
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    monthlyRows.push({ label, value: Math.round(Number(mResult.rows[0].total)) });
  }

  const avgMonthlySpending = monthlyRows.length > 0
    ? Math.round(monthlyRows.reduce((s, r) => s + r.value, 0) / monthlyRows.length)
    : 0;
  ...
  const weekdayAvg = weekdayCount > 0
    ? dailyBreakdown.filter((_, i) => i < 5).reduce((s, e) => s + e.value, 0) / weekdayCount
    : 0;
